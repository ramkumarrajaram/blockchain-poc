...
void ZkTransfer_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void ZkTransfer_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 264;}

uint get_number_of_components() {return 4;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 255;}
...
ctx->componentMemory[coffset].templateName = "ZkTransfer";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint csoffset = mySignalStart+3;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "balanceComparator";
...
ZkTransfer_3_create(1,0,ctx,"main",0);
ZkTransfer_3_run(0,ctx);
}
