...
void PublicKey_11_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void PublicKey_11_run(uint ctx_index,Circom_CalcWit* ctx);
...
void ZkIdentity_14_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void ZkIdentity_14_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 10144;}

uint get_number_of_components() {return 961;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 788;}
...
ctx->componentMemory[coffset].templateName = "PublicKey";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
ctx->componentMemory[coffset].templateName = "ZkIdentity";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 5;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "publicKey";
PublicKey_11_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 10114 ;
aux_cmp_num += 952;
}
...
for (uint i = 0; i < 4; i++) {
std::string new_cmp_name = "equals"+ctx->generate_position_array(aux_dimensions, 2, i);
...
// need to run sub component
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1));
PublicKey_11_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &lvar[1];
// load src
...
Fr_lt(&expaux[0],&lvar[2],&circuitConstants[8]); // line circom 45
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = (((2 * Fr_toInt(&lvar[2])) + 0) + 1);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
...
{
uint cmp_index_ref = (((2 * Fr_toInt(&lvar[2])) + 1) + 1);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[(((2 * Fr_toInt(&lvar[2])) + 0) + 1)]].signalStart + 0]); // line circom 55
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[(((2 * Fr_toInt(&lvar[2])) + 1) + 1)]].signalStart + 0]); // line circom 56
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_eq(&expaux[0],&lvar[1],&circuitConstants[8]); // line circom 61
...
ZkIdentity_14_create(1,0,ctx,"main",0);
ZkIdentity_14_run(0,ctx);
}
